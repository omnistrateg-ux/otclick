...
# ============================================================================
# Knowledge/Simulation - Reference Data
# ============================================================================


@router.get("/reference/knowledge-categories")
async def get_knowledge_categories() -> list[dict[str, str]]:
    """Get knowledge categories."""
    return [{"category": c.value} for c in KnowledgeCategory]


@router.get("/reference/knowledge-confidences")
async def get_knowledge_confidences() -> list[dict[str, str]]:
    """Get knowledge confidence levels."""
    return [{"confidence": c.value} for c in KnowledgeConfidence]


@router.get("/reference/pattern-types")
async def get_pattern_types() -> list[dict[str, str]]:
    """Get pattern types."""
    return [{"type": t.value} for t in PatternType]


@router.get("/reference/simulation-outcomes")
async def get_simulation_outcomes() -> list[dict[str, str]]:
    """Get simulation outcomes."""
    return [{"outcome": o.value} for o in SimulationOutcome]


@router.get("/reference/conflict-severities")
async def get_conflict_severities() -> list[dict[str, str]]:
    """Get conflict severities."""
    return [{"severity": s.value} for s in ConflictSeverity]


@router.get("/reference/training-types")
async def get_training_types() -> list[dict[str, str]]:
    """Get training scenario types."""
    return [{"type": t.value} for t in TrainingScenarioType]


@router.get("/reference/training-difficulties")
async def get_training_difficulties() -> list[dict[str, str]]:
    """Get training difficulties."""
    return [{"difficulty": d.value} for d in TrainingDifficulty]
